...
public:
    ...
    double parameters_[7] = {1, 0, 0, 0, 0, 0, 0};
    Eigen::Map<Eigen::Quaterniond> q_local_curr = Eigen::Map<Eigen::Quaterniond>(parameters_);
    Eigen::Map<Eigen::Vector3d> t_local_curr = Eigen::Map<Eigen::Vector3d>(parameters_ + 4);

    pcl::KdTreeFLANN<pcl::PointXYZI>::Ptr kdtreeEdgesubMap;
    pcl::KdTreeFLANN<pcl::PointXYZI>::Ptr kdtreeSurfsubMap;
    pcl::PointCloud<PointType>::Ptr cureKeyframeCloud_corner;
    pcl::PointCloud<PointType>::Ptr prevKeyframeCloud_corner;
    pcl::PointCloud<PointType>::Ptr cureKeyframeCloud_surface;
    pcl::PointCloud<PointType>::Ptr prevKeyframeCloud_surface;
...
public:
    ...
    gtsam::Pose3 Pose6DtoGTSAMPose3(const PointTypePose *const transformIn)
    {
        return gtsam::Pose3(gtsam::Rot3::RzRyRx(transformIn->roll, transformIn->pitch, transformIn->yaw),
                            gtsam::Point3(transformIn->x, transformIn->y, transformIn->z));
    } // Pose6DtoGTSAMPose3 --> SlamCabbage
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    void loopClosureThread()
    {
        if (loopClosureEnableFlag == false)
            return;
        const std::string loopConstraintTime = savePCDDirectory + "/loopConstraintTime.txt";
        std::ofstream f;
        f.open(loopConstraintTime, std::ios::out);
        ros::Rate rate(loopClosureFrequency); // 1hz
        while (ros::ok())
        {
            rate.sleep();
            tic = std::chrono::system_clock::now();
            performNDTMCLoopClosure();
            ...
            f << elapsed_ms << endl;
            cout << "time of loop : " << elapsed_ms << endl;
    ...
    void GetPoints(PointType const *const pi, PointType *const po)
    {
        ...
        for (int i = 0; i < (int)pc_in->points.size(); i++)
        {
            ...
            if (pointSearchSqDis[4] < 1.0)
            {
                ...
                if (saes.eigenvalues()[2] > 3 * saes.eigenvalues()[1])                                  // 当最大的特征值大于3倍第二大的特征值，则认为是一个直线
                {
                    ...
                    problem.AddResidualBlock(cost_function, loss_function, parameters_);                             // 添加残差块，其中parameters是代优化的参数
                    corner_num++;
                }
            }
        }
        LOG(INFO) << "corner_num : " << corner_num;
        //    if(corner_num<10){
    ...
    void addSurfCostFactor(const pcl::PointCloud<PointType>::Ptr &pc_in, const pcl::PointCloud<PointType>::Ptr &map_in, ceres::Problem &problem, ceres::LossFunction *loss_function)
    {
        ...
        LOG(INFO) << "sur_num : " << surf_num;
        //    if(surf_num<20){
        //        cout <<"not enough correct points" << endl;
        //    }
    }

    gtsam::Pose3 doCeres(gtsam::Pose3 &curr2loop)
    {
        int optimization_count = 2;
        // 初始化parameters，点云均在世界坐标系下
        parameters_[0] = 1;
        parameters_[1] = 0;
        parameters_[2] = 0;
        parameters_[3] = 0;
        ...
        parameters_[5] = 0;
        parameters_[6] = 0;
        // parameters_[3] = curr2loop.x();
        // parameters_[4] = curr2loop.y();
        // parameters_[5] = curr2loop.z();
        ...
        kdtreeEdgesubMap->setInputCloud(prevKeyframeCloud_corner);
        kdtreeSurfsubMap->setInputCloud(prevKeyframeCloud_surface);
        for (int iterCount = 0; iterCount < optimization_count; iterCount++)
        {
            ...
            addEdgeCostFactor(cureKeyframeCloud_corner, prevKeyframeCloud_corner, problem, loss_function);
            addSurfCostFactor(cureKeyframeCloud_surface, prevKeyframeCloud_surface, problem, loss_function);
        ...
        poseFrom = gtsam::Pose3(gtsam::Rot3::RzRyRx(eulerAngle[0], eulerAngle[1], eulerAngle[2]), gtsam::Point3(t_local_curr[0], t_local_curr[1], t_local_curr[2]));
    ...
    void performNDTMCLoopClosure()
    {
        ...
        *copy_cloudKeyPoses6D = *cloudKeyPoses6D;
        ...
        // find keys
        // auto detectResult = scManager.detectLoopClosureID(); // first: nn index, second: yaw diff
        auto detectResult = NmcManager.detectLoopClosureID(copy_cloudKeyPoses3D); // first: nn index, second: yaw diff
        int loopKeyCur = copy_cloudKeyPoses3D->size() - 1;
        int loopKeyPre = detectResult.first;
        float yawDiffRad = detectResult.second; // not use for v1 (because pcl icp withi initial somthing wrong...)
        if (loopKeyPre == -1 /* No loop found */)
            return;
        // distance threshold
        gtsam::Pose3 curr_gtPose3 = Pose6DtoGTSAMPose3(&(copy_cloudKeyPoses6D->points[loopKeyCur]));
        gtsam::Pose3 loop_gtPose3 = Pose6DtoGTSAMPose3(&(copy_cloudKeyPoses6D->points[loopKeyPre]));
        gtsam::Pose3 curr2loop = curr_gtPose3.between(loop_gtPose3);
        // int num_of_keyframe = copy_cloudKeyPoses6D->points.size();
        // static int ll;
        ...
        //     ll++;
        // }
        double odom_distance = sqrt((curr2loop.x()) * (curr2loop.x()) + (curr2loop.y()) * (curr2loop.y()));
        // if (odom_distance > 30 + ll / 20 / 40 /* No loop found */)
        // {
        //     LOG(INFO) << "[FILED] distance is larger than distance threshold :" << loopKeyCur << " and " << loopKeyPre;
        //     LOG(INFO) << "distancd : " << odom_distance;
        //     return;
        // }

        // std::cout << "loop found! between " << loopKeyCur << " and " << loopKeyPre << "." << " distance: " << odom_distance << std::endl; // giseop

        LOG(INFO) << "[SUCCESSE] distance find loop : " << loopKeyCur << " and " << loopKeyPre << " distance: " << odom_distance;

        // // extract cloud, corner和surface按不同的容器存储
        // cureKeyframeCloud_corner = pcl::PointCloud<PointType>::Ptr(new pcl::PointCloud<PointType>());
        // prevKeyframeCloud_corner = pcl::PointCloud<PointType>::Ptr(new pcl::PointCloud<PointType>());
        // cureKeyframeCloud_surface = pcl::PointCloud<PointType>::Ptr(new pcl::PointCloud<PointType>());
        // prevKeyframeCloud_surface = pcl::PointCloud<PointType>::Ptr(new pcl::PointCloud<PointType>());
        // LOG(INFO) << "initial cureKeyframeCloud_corner success";
        // {
        //     // loopFindNearKeyframesWithRespectTo(cureKeyframeCloud, loopKeyCur, 0, loopKeyPre); // giseop
        //     // loopFindNearKeyframes(prevKeyframeCloud, loopKeyPre, historyKeyframeSearchNum);
        ...
        //     // loopFindNearKeyframesWithRespectTo(cureKeyframeCloud, loopKeyCur, 0, base_key);                        // giseop
        //     // loopFindNearKeyframesWithRespectTo(prevKeyframeCloud, loopKeyPre, historyKeyframeSearchNum, base_key); // giseop
        //     loopFindNearKeyframesWithRespectTo_corner(cureKeyframeCloud_corner, loopKeyCur, 0, base_key);
        //     loopFindNearKeyframesWithRespectTo_surface(cureKeyframeCloud_surface, loopKeyCur, 0, base_key);
        //     loopFindNearKeyframesWithRespectTo_corner(prevKeyframeCloud_corner, loopKeyPre, historyKeyframeSearchNum, base_key);
        //     loopFindNearKeyframesWithRespectTo_surface(prevKeyframeCloud_surface, loopKeyPre, historyKeyframeSearchNum, base_key);
        //     LOG(INFO) << "find feature points size : " << cureKeyframeCloud_corner->size() + cureKeyframeCloud_surface->size() << " and " << prevKeyframeCloud_corner->size() + prevKeyframeCloud_surface->size();

        //     if (cureKeyframeCloud_corner->size() + cureKeyframeCloud_surface->size() < 300 || prevKeyframeCloud_corner->size() + prevKeyframeCloud_surface->size() < 1000)
        //         return;

        //     if (pubHistoryKeyFrames.getNumSubscribers() != 0)
        //     {
        //         pcl::PointCloud<PointType>::Ptr prevKeyframeCloud_(new pcl::PointCloud<PointType>());
        //         *prevKeyframeCloud_ += *prevKeyframeCloud_corner;
        //         *prevKeyframeCloud_ += *prevKeyframeCloud_surface;
        //         publishCloud(&pubHistoryKeyFrames, prevKeyframeCloud_, timeLaserInfoStamp, odometryFrame);
        //     }
        // }

        // gtsam::Pose3 poseFrom = doCeres(curr2loop);
        // gtsam::Pose3 poseTo = gtsam::Pose3(gtsam::Rot3::RzRyRx(0.0, 0.0, 0.0), gtsam::Point3(0.0, 0.0, 0.0));
        // Eigen::Matrix3d rot = Eigen::Matrix3d(poseFrom.rotation().matrix().data());
        // Eigen::Vector3d trans = Eigen::Vector3d(poseFrom.translation().data());

        // LOG(INFO) << "curr2loop : " << curr2loop;
        // LOG(INFO) << "poseFrom : " << poseFrom;

        // // Create an Eigen::Affine3d object from rotation and translation components
        // Eigen::Affine3d affine(rot);
        // affine.translation() = trans;

        // // Get pose transformation
        // // float x, y, z, roll, pitch, yaw;
        // // Eigen::Affine3d pose = relative_pose.cast<double>();
        // // Eigen::Affine3f correctionLidarFrame = pose.cast<float>();
        ...
        // {
        //     pcl::PointCloud<PointType>::Ptr closed_cloud(new pcl::PointCloud<PointType>());
        //     pcl::transformPointCloud(*cureKeyframeCloud_corner + *cureKeyframeCloud_surface, *closed_cloud, affine);
        //     publishCloud(&pubIcpKeyFrames, closed_cloud, timeLaserInfoStamp, odometryFrame);
        // }
        ...
        // loopNoiseQueue.push_back(robustConstraintNoise);
        // mtx.unlock();
        // LOG(INFO) << "success add loop constraint";
        // LOG(INFO) << "";
    ...
    void loopFindNearKeyframesWithRespectTo_corner(pcl::PointCloud<PointType>::Ptr &nearKeyframes, const int &key, const int &searchNum, const int _wrt_key)
    {
        // extract near keyframes
        nearKeyframes->clear();
        int cloudSize = copy_cloudKeyPoses6D->size();
    ...
    void loopFindNearKeyframesWithRespectTo_surface(pcl::PointCloud<PointType>::Ptr &nearKeyframes, const int &key, const int &searchNum, const int _wrt_key)
    {
        // extract near keyframes
        nearKeyframes->clear();
        int cloudSize = copy_cloudKeyPoses6D->size();
    ...
    void visualizeLoopClosure()
    {
        ...
        const std::string loopConstraint = savePCDDirectory + "/loopConstraint.txt";
        std::ofstream ff;
        ff.open(loopConstraint, std::ios::out);
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    void addLoopFactor()
    {
        ...
        for (int i = 0; i < (int)loopIndexQueue.size(); ++i)
        {
            ...
            LOG(INFO) << "add loop factor : " << indexFrom << " " << indexTo;
    ...
    void saveKeyFramesAndFactor()
    {
        ...
        if (sc_input_type == SCInputType::SINGLE_SCAN_FULL)
        {
            ...
            NmcManager.makeAndSaveNdtMapCodeAndKeys(*thisRawCloudKeyFrame);
        }
        else if (sc_input_type == SCInputType::SINGLE_SCAN_FEAT)
        {
            scManager.makeAndSaveScancontextAndKeys(*thisSurfKeyFrame);
            // NmcManager.makeAndSaveNdtMapCodeAndKeys(*thisSurfKeyFrame);
        }
        else if (sc_input_type == SCInputType::MULTI_SCAN_FEAT)
        {
            ...
            NmcManager.makeAndSaveNdtMapCodeAndKeys(*multiKeyFrameFeatureCloud);
        }
        // else if (sc_input_type == SCInputType::MULTI_SCAN_RAW){
        //     pcl::PointCloud<PointType>::Ptr multiKeyFrameFeatureCloud(new pcl::PointCloud<PointType>());
        //     // Todo:
        //     // 需要在这里加入：rawCloudKeyFrames
        //     loopFindNearKeyframes(multiKeyFrameFeatureCloud, cloudKeyPoses6D->size() - 1, historyKeyframeSearchNum);
        //     // scManager.makeAndSaveScancontextAndKeys(*multiKeyFrameFeatureCloud);
        //     NmcManager.makeAndSaveNdtMapCodeAndKeys(*multiKeyFrameFeatureCloud);
        // }
        ...
        // const auto &curr_scd = NmcManager.getConstRefRecentSCD();
        // std::string curr_scd_node_idx = padZeros(scManager.polarcontexts_.size() - 1);
        std::string curr_scd_node_idx = padZeros(NmcManager.polarcontexts_.size() - 1);
...
int main(int argc, char **argv)
{
    ...
    FLAGS_log_dir = "/home/liaolizhou/liosam/Myproject/src/SC-LIO-SAM/log";
